...
- Test gaps: the unit/page tests mock the API client, so frontend/backend contract drift is caught only by the shared `@cadence/schema` types and the e2e suite. The e2e suite is local-only (needs Redis, a worker with ML dependencies, ffmpeg and real videos, so it can't run in CI) and drives Edge only. It now covers the happy path, auth, the dancer-pick flow, a no-person video being rejected, a non-YouTube link being refused, and an automated no-sideways-scroll check at 390/768/1440px -- but not the YouTube-link *happy path* (it would download a real video), a rejected *reference* video, or other browsers. No visual-regression tests.
