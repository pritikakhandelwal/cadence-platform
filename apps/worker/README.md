...
- `scoring.py` — segments the warping path into ~2s windows (in the reference video's own timeline) and produces up to six issue types per window:
  ...
  - `tempo` — the DTW path locally stretched or compressed far more than normal here, a proxy for a move likely skipped/rushed or added/repeated.
  - `energy` — overall limb movement in this window is under half the reference's (only checked when the reference itself has enough motion to compare against).
  - `balance` — hip position wanders from the ankle-to-ankle base of support more than the reference's own baseline.
