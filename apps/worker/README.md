...
- `extract_locked_pose_job(analysis_id, video_path, track_id, fps, total_frames, detections)` — resumes from stashed detections once a track is picked, without re-running YOLO. Enqueued by `apps/api`'s `POST /analyses/{id}/lock`. Writes the same final `complete`/`rejected` `AnalysisResult` shape as `detect_tracks_job`'s auto-lock path, and clears `pending_lock_data` once done.
...
Phase 2 (this pipeline) covers detect→track→lock→quality-gate, and both paths are now wired end-to-end: the auto-lock path (upload → enqueue → detect → lock → persisted result) and the `needs_dancer_pick` path (`apps/api`'s `GET /analyses/{id}/candidate-tracks` + `POST /analyses/{id}/lock` → this app's `extract_locked_pose_job`). Still missing: any actual UI to call the pick endpoints from (`apps/web` calls none of this), and a real eval dataset (roadmap's "film 30 clips yourself" — MOTA/IDF1 numbers need that, and don't exist yet; two synthetic ffmpeg-composited clips is a spot-check, not that battery). Phase 3 (align → score → segment) is next. See [`../../STATUS.md`](../../STATUS.md).
