...
Ported from legacy/cadence-streamlit/modules/input_validation.py — this
logic was framework-agnostic already, so it moves over unchanged.
"""
...
def validate_name(name: str) -> ValidationResult:
    ...
    if len(stripped) > NAME_MAX:
        return ValidationResult.bad(f"Name must be {NAME_MAX} characters or fewer.")
...
def validate_email(email: str) -> ValidationResult:
    ...
    if len(stripped) > EMAIL_MAX:
        return ValidationResult.bad(f"Email must be {EMAIL_MAX} characters or fewer.")
...
def validate_registration_fields(
    ...
    if len(password) < password_minimum:
        return ValidationResult.bad(f"Password must be at least {password_minimum} characters.")
