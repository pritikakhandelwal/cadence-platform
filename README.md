...
Phases 0, 1, 2, and 3 are all done and wired end-to-end: upload → `apps/api` enqueues a job → `apps/worker` detects/tracks/locks a dancer (or asks the API to ask the user to pick one, for a duo/mirror clip) → extracts pose → aligns it to the reference video with DTW → scores it → writes a real `overall` score and timestamped, joint-level `segments`/`issues` into the same row `apps/api` reads back. Verified against real footage, not just unit-tested in isolation — including a synthetic planted-error eval that took three redesigns to get honest (see `apps/worker/README.md`), and a real, documented limitation (2D pose can't distinguish a genuine angle difference from a camera-viewpoint difference — see `docs/decisions.md`). What's still missing: any frontend calling any of this — `apps/web` is still the Phase 0 placeholder — and a real eval dataset beyond synthetic stress clips and one real-but-unlabeled clip pair. Read [`STATUS.md`](STATUS.md) before starting work on any phase; it tracks what's actually done vs. planned, since the two drift.
