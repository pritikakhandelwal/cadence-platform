...
_Last updated: 2026-09-16 (Phase 3 built and wired end-to-end; both Phase 2 paths now score against the reference video)._
...
- [ ] Still no frontend. `apps/web` doesn't call any of Phase 2 — a user cannot actually trigger any of this yet, only `apps/api`'s own tests can. The three endpoints (`POST /analyses`, `GET .../candidate-tracks`, `POST .../lock`) are the *complete* multi-person flow now, API-side; there's just nothing calling them.
...
- [ ] No real eval dataset — the roadmap's "10 phrase pairs, good vs. known-bad, >=70% planted-error hit rate" needs *real* filmed pairs with a *known* planted error. What exists is a synthetic proxy for that methodology (see above) plus one real-but-unlabeled clip pair. Different things; don't conflate them.
- [ ] Alembic migrations still pending (same item as Phase 1, unchanged).

**What's real here:** the full pipeline runs against real footage end-to-end and writes a real, inspectable `overall` score and timestamped `segments` with joint-level issues into the database, and the scoring logic itself has been checked against a synthetic ground truth that meets the roadmap's own accuracy bar. **What's not real yet:** there is no real eval set, the 2D-viewpoint limitation is confirmed but unresolved, and — as with every phase so far — nobody outside a test can trigger any of this, because `apps/web` still calls nothing.
...
Phases 2 and 3 are both now wired end-to-end (upload -> lock -> score -> persisted result), which is the first point in this build where `apps/web` calling the API would actually have something real to show. That makes Phase 4 (Arena UI) a legitimate next step for the first time — but it's not the only reasonable one:

1. **Start Phase 4** (Arena UI): there's finally a real `overall` score and timestamped `segments`/`issues` to build a screen around, which is exactly the precondition the roadmap's own risk note (Section 7) requires before touching the frontend.
2. **Eval breadth first**: everything scored so far is one real (unlabeled) clip pair plus synthetic stress tests. A few more real clip pairs — including ones with a *known* planted error, per the roadmap's actual eval methodology — would tell us whether the 2D-viewpoint limitation and the 15-degree issue threshold are reasonable in general or just on one example.

Given the user's own direction to "fix this step by step" and that Phase 4 is explicitly what makes the product visible and usable, Phase 4 is the more likely next step — but eval breadth is the more scientifically honest one, and doing Phase 4 without it means the UI will confidently display numbers nobody has checked against more than one data point.
