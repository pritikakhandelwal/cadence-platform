...
**Trigger:** `init_db()`'s own docstring said to move to Alembic "if the schema needs to evolve without dropping data" -- and the timezone fix (see the entry above) was exactly that kind of change, with no way to apply it to an existing database.

**Decision:** migrations live inside the `cadence_db` package (so a built wheel carries them) with an `alembic.ini` in `packages/db` for the CLI; the URL comes from `DATABASE_URL` like the apps, or an explicit one for tests. `create_all` stays for dev and tests. That leaves two sources of truth for the schema, which is a known way to rot -- so `apps/api/tests/test_migrations.py` asserts that `upgrade head` on an empty database produces *exactly* the models' schema (`compare_metadata` with type comparison), that there is one head, that downgrade removes everything, and that upgrading twice is a no-op. A model change without a migration fails it.

**Why not switch the apps to run migrations at startup:** that changes deploy semantics (who applies a migration, and when, in a multi-instance deploy) and nobody has decided that yet; and a database built by one method can't be adopted by the other without care (a `create_all` database has no `alembic_version`). Recorded rather than papered over -- deciding it is a Phase 7 (production hardening) question.

**Verified vs. not:** baseline autogenerated against real PostgreSQL 16 and reviewed by hand; the guard tests mutation-checked (unmigrated column, naive datetime on Postgres, second head); CLI exercised for real. Not done: any data migration, an upgrade from a *previous* revision (there isn't one yet), or applying migrations to a database that already holds data.
