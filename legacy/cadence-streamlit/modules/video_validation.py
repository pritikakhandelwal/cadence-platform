...
def _check_mp4_magic_bytes(upload: object) -> bool:
    """Return True if the upload buffer starts with an ISO Base Media ftyp box."""
    try:
        buf = upload.getbuffer()
        header = bytes(buf[_MP4_MAGIC_OFFSET: _MP4_MAGIC_OFFSET + len(_MP4_MAGIC)])
        return header == _MP4_MAGIC
    except Exception:
        return False


def validate_uploaded_video(upload: object) -> None:
    ...
    if size is None and hasattr(upload, "getbuffer"):
    ...
    if hasattr(upload, "getbuffer") and not _check_mp4_magic_bytes(upload):
